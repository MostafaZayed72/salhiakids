...
const newStory = ref({
  ...
  publishYear: null, // 👈 التعديل الأول: سنة النشر
})
...
const handleFileSelected = async (event, targetField) => {
  ...
  if (url) {
    ...
    alert('فشل رفع الملف. الرجاء التأكد من أن الملف صورة.')
  }
...
const addStory = async () => {
  ...
  if (!catId || isUploading.value || !newStory.value.title || !newStory.value.content) {
    alert('الرجاء ملء الحقول المطلوبة (بما في ذلك العنوان والمحتوى) والتأكد من عدم وجود عملية رفع قيد التنفيذ.')
    return
  }
...
<div class="w-full h-40 mb-4 rounded-xl overflow-hidden bg-gray-100 shadow-inner relative">
 ...
 <div class="relative bg-white rounded-3xl p-6 w-full max-w-2xl mx-4 shadow-2xl transform transition">
  ...
  <div v-else-if="newStory.mediaType === 2">
 ...
 <div class="relative bg-white rounded-3xl p-6 w-full max-w-2xl mx-4 shadow-2xl transform transition">
  ...
  <div>
   ...
   class="w-full rounded-md border border-gray-300 shadow-sm px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"></textarea>
    ...
    <div>
   <label class="block mb-1 text-sm font-medium text-gray-700">سنة النشر (اختياري)</label>
...
<div>
  ...
  <div v-else-if="editingStory.mediaType === 2">
   <label class="block mb-1 text-sm font-medium text-gray-700">رابط فيديو YouTube</label>
