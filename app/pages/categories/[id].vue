...
const truncateText = (htmlContent, maxLength) => {
  ...
  // 4. (تبسيط): نستخدم النص الخام المقصوص، ونضيف علامة "..."
  // *ملاحظة:* القص مع المحافظة على وسم HTML يتطلب مكتبة معقدة. لهذا التبسيط، سنكتفي بالنص المقصوص.
   ...
   <section class="py-12 bg-gradient-to-br from-purple-300 via-pink-400 to-purple-400 ">
      <div class="container mx-auto px-4">
         <div class="text-center mb-12">
            ...
            <div class="flex flex-col md:flex-row justify-center items-center gap-4 mt-6">
              ...
              class="flex items-center rounded-2xl border-2 border-indigo-400 focus-within:border-red-500 transition-all duration-300 bg-white">
                ...
                @keyup.enter="handleSearch"         placeholder="ابحث بعنوان القصة..."        
         ...
         <div v-if="isLoading" class="text-center text-gray-700 py-20">
            <div
          class="animate-spin rounded-full h-12 w-12 border-4 border-purple-600 border-t-transparent mx-auto"></div>
        ...
        class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 max-w-7xl mx-auto ">
          ...
          class="group relative p-4 rounded-2xl bg-gradient-to-br from-purple-300 via-pink-300 to-purple-600 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 shadow-md border border-gray-200 cursor-pointer overflow-hidden"
            ...
            class="absolute top-3 left-3 flex gap-2 z-10 opacity-0 group-hover:opacity-100 transition">
              ...
              class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                  <div v-else
              class="w-full h-full flex items-center justify-center text-gray-400 text-3xl font-semibold">
         ...
         <div v-else
        class="text-center py-10 bg-white rounded-xl shadow-lg border-2 border-dashed border-purple-300">
            ...
            <form @submit.prevent="addStory" class="mt-4 space-y-4">
              ...
              class="w-full text-sm text-gray-600" />
                  <div v-if="newStory.imageUrl" class="mt-3">
                     <img :src="newStory.imageUrl" alt="preview"
            ...
            <form @submit.prevent="updateStory" class="mt-4 space-y-4">
              ...
              class="w-full text-sm text-gray-600" />
                  <div v-if="editingStory.imageUrl" class="mt-3">
                     <img :src="editingStory.imageUrl" alt="preview"
                class="w-40 h-24 object-cover rounded-lg border">
                    </div>
                 </div>
      ...
      <Transition name="modal">
        ...
        @click.self="showReviewModal = false"  >
            <div
          class="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-sm mx-4 transform transition-all duration-300">
