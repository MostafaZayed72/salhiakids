...
// ------------------------------------
// ------------------------------------
// 4. النشر (Approve) - استخدام PATCH على Endpoint الجديد
// ------------------------------------
const publishStory = async (story) => {
if (!confirm(`هل أنت متأكد من نشر القصة: ${story.title}؟`)) return
...
try {
   ...
   await fetchUnpublishedStories() // تحديث القائمة لإزالة القصة المنشورة
} catch (err) {
   console.error('Publish story failed:', err.response?.data || err)
   alert('فشل نشر القصة: ' + (err.response?.data?.message || 'خطأ غير معروف'))
}
}
     ...
     <div v-else-if="stories.length > 0" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 max-w-7xl mx-auto ">
      ...
      >
       ...
       <div class="w-full h-40 mb-4 rounded-xl overflow-hidden bg-gray-100 shadow-inner">
        ...
        <div v-else class="w-full h-full flex items-center justify-center text-red-400 text-3xl font-semibold">
         <span class="material-icons text-5xl">unpublished</span>
        </div>
       </div>
