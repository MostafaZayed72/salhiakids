...
// 3. دالة جلب بيانات المستخدم مع Bearer Token (تم تعديلها لاستقبال جميع المستخدمين)
const fetchUser = async () => {
 ...
 try {
  ...
  const user = await response.json()
    ...
    // إعداد حقول Name و Role و FirstInitial لحفظها في localStorage
    const defaultFullName = `${user.firstName || ''} ${user.lastName || ''}`.trim();
  const fullName = user.name || user.displayName || defaultFullName || 'مستخدم';
    const userRole = (user.userTypeName || (user.roles && user.roles[0]) || 'user').toLowerCase();
    const firstInitial = user.firstName ? user.firstName.charAt(0) : (fullName.charAt(0) || 'م');
    ...
    // عرض بيانات المستخدم وحفظها (لأي دور)
  userData.value = user
  localStorage.setItem('userData', JSON.stringify(userToStore))
    
    // إطلاق حدث لتحديث الـ Header فوراً
    window.dispatchEvent(new Event('auth-change')); 
 ...
 <div dir="rtl" :class="appearanceClasses">
  ...
  <div class="container mx-auto px-4 py-8" :class="containerClasses">
    ...
    <div class="max-w-4xl mx-auto">
          <h1 class="text-3xl font-bold mb-8 text-gray-800">الملف الشخصي</h1>
   ...
   <div v-else-if="!isAuthenticated" class="text-center py-20 bg-white rounded-xl shadow-lg">
    ...
    <p class="text-gray-600">الرجاء تسجيل الدخول للوصول إلى الملف الشخصي.</p>
